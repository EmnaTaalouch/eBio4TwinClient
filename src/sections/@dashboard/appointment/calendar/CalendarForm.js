...
// @mui
import { Box, Stack, Button, Tooltip, TextField, IconButton, DialogActions, Autocomplete, FormHelperText, styled, Typography } from '@mui/material';
import { DatePicker, LoadingButton, MobileDateTimePicker, TimePicker } from '@mui/lab';
// redux
import { useEffect, useState } from 'react';
import { useFormik } from 'formik';
import { useDispatch, useSelector } from '../../../../redux/store';
import { createAppointmentFromCalendar, updateAppointmentFromCalendar, deleteAppointmentFromCalendae } from '../../../../redux/slices/appointmentSlice';
// components
...
export default function CalendarForm({ event, range, onCancel }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formData) => {
      try {
        if (event.id) {
          dispatch(updateAppointmentFromCalendar(event.id, {...formData,dateApt:DateFix(formData.dateApt),nutritionist:currentUser}));
          ...
          dispatch(createAppointmentFromCalendar({...formData,dateApt:DateFix(formData.dateApt),nutritionist:currentUser}));
        }
        formik.handleReset();
        onCancel();
